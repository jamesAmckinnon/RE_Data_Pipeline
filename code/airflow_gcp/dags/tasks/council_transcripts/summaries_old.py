def transcript_vectors_to_summaries():
    ...
    ####################    FOR TESTING    #################### &&&&
    # transcripts = transcripts[0:2]
    random_num_1 = random.randint(2, len(transcripts)-1)
    transcripts = [transcripts[random_num_1]]
    ###########################################################
    ...
    real_estate_query = (
        "zoning changes, bylaw changes, rezoning approvals/denials, commercial property, "
        "land use planning, property tax rules, development approvals, commercial real estate"
    )
    ...
    for transcript in transcripts:
        ...
        print(f"Retrieved {len(docs)} chunks for transcript {transcript.council_transcript_id}.")
        docs_with_sources = []
        for d in docs:
        ...
        combine_prompt = PromptTemplate(
            template=(
                "Combine the following partial summaries into a single coherent markdown report. Extract"
                "important information that could impact the commercial real estate industry, including zoning"
                "and bylaw changes, property tax discussions, general discussion about real estate or topics"
                ...
                "and any arguments for or against the changes. Only include information that was directly"
                "discussed in the text and include inline hyperlinked source citations (Eg. [[1]](the YouTube Link)."
                "\n\n{text}\n\nFinal Summary:"
            ),
            input_variables=["text"],
        )
        ...
        summaries[transcript.council_transcript_id] = summary
