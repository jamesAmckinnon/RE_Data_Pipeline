def get_AV_listings(city_name, gcs_bucket, gcs_path):
    import sys
    import pandas as pd
    import json
    import requests
    ...
    if response.status_code == 200:
        ...
        all_properties_objects = [prp for prp in all_properties_objects if prp["city"] == city_name and prp["status"] != "closed"]
        all_properties_objects = all_properties_objects
        ...
        listings_df["latitude"] = listings_df["location"].apply(lambda x: round(float(x["lat"]), 10))
        listings_df["longitude"] = listings_df["location"].apply(lambda x: round(float(x["lng"]), 10))
