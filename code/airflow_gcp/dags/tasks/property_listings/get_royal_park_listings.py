def get_royal_park_listings(gcs_bucket, gcs_path):
    ...
    if response.status_code == 200:
        data = response.json()  
        all_properties_objects = data["properties"]
        ...
        listings_df["latitude"] = round(listings_df["latitude"].astype(float), 10)
        listings_df["longitude"] = round(listings_df["longitude"].astype(float), 10)
